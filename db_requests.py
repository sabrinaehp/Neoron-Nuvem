...
class LocalDB:
    ...
    def select_table_dual_where(
        self, table_name: str, column: str, value: str, column2: str, value2: str
    ) -> list[tuple[str]]:
        insert_sql = f"select * from {table_name} WHERE {column} = '{value}' AND {column2} = '{value2}'"
        self.__open_connection()
        self.cursor.execute(insert_sql)
        result = self.cursor.fetchall()
        self.__close_connection()
    ...
    def unique_key(self, table_name: str, column: str) -> str:
        while True:
...
if __name__ == "__main__":
    ...
    db = LocalDB()
    db.create_table(TABLE_LOCALIDADES, COLUMNS_LOCALIDADES)
    db.create_table(TABLE_VOOS, COLUMNS_VOOS)
